...
public class ExcelCleaner {
   ...
   public static void main(String[] args) {
    ...
    //Map<String, List<List<Cell>>> complete_Raw = extractInfo_Raw(new File("C:\\workspace2\\Tools\\src\\com\\ericsson\\utran\\tools\\intern\\ISF117033_LTE_RND_T-Mobile_RND_CIQ_MMBB_02142022.xlsx"));
   	...
   	File inputFile = new File("C:\\workspace2\\Tools\\src\\com\\ericsson\\utran\\tools\\intern\\ISF117033_LTE_RND_T-Mobile_RND_CIQ_MMBB_02142022.xlsx");
   	File outputFile = new File("C:\\workspace2\\Tools\\src\\com\\ericsson\\utran\\tools\\intern\\deleted_empty_rows_info.xlsx");
   	
    boolean isDone = ExcelCleaner.clean(inputFile, outputFile);
   ...
   public static boolean clean(File inputFile, File outputFile)
   {
	...
	try {
	   ...
	   try
	   {
		   inputBook = new XSSFWorkbook(new FileInputStream(inputFile));
		   outputBook = new XSSFWorkbook();

		   
		   int sheetCount = inputBook.getNumberOfSheets();	//number of sheets in the excel file
		   ...
		   for(int currSheet=0; currSheet<sheetCount; currSheet++)
	         {
	            ...
	            Sheet sheet = inputBook.getSheetAt(currSheet);
	            ...
	            //output
	            Sheet outputSheet = outputBook.createSheet(sheetName);
	            
	            //traverse rows
	            for (int i = 0; i < infoList_Raw.size(); i++) 
	            {
	               Row outputRow = outputSheet.createRow(i);
	               System.out.print("Row #"+(i+1)+": ");
	            
	            	//traverse columns for that particular row
	               for (int j = 0; j < infoList_Raw.get(i).size(); j++) 
	               {
	                  Cell outputCell = outputRow.createCell(j);
	                  Cell rawCell=infoList_Raw.get(i).get(j);
	               	
	                  outputCell.setCellType(rawCell.getCellType());
	               	
	                  switch (rawCell.getCellType()) {
	                     case Cell.CELL_TYPE_NUMERIC:
	                    	 outputCell.setCellValue(Math.round(rawCell.getNumericCellValue()));
	                        break;
	                     case Cell.CELL_TYPE_STRING:
	                    	 outputCell.setCellValue(rawCell.getStringCellValue());
	                        break;
	                     case Cell.CELL_TYPE_BOOLEAN:
	                    	 outputCell.setCellValue(rawCell.getBooleanCellValue());
	                        break;
	                     case Cell.CELL_TYPE_FORMULA:
	                    	 outputCell.setCellValue(rawCell.getCellFormula());
	                        break;
	                     default:
	                    	 outputCell.setCellValue("phineas");
	                        break;
	                  }
	                  System.out.print(outputCell.toString()+", ");//print values in current row
	               }
